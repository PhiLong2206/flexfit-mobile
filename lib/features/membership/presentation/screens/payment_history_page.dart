...
class _PaymentHistoryCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final effectiveStatus = _effectivePaymentStatus(payment);
    debugPrint(
      'Raw payment status: ${payment.status}, Mapped: ${_statusLabel(effectiveStatus)}',
    );
    final badgeColor = _statusColor(effectiveStatus);
...
Color _statusColor(String status) {
  ...
  if (value.contains('fail') || value.contains('error') || value == 'failure') {
    return AppColors.cancelled;
  }
  if (value == 'pending' || value == 'processing') {
    return AppConstants.primaryColor;
  }
  return AppConstants.primaryColor; // Fallback
}
...
String _effectivePaymentStatus(PaymentHistoryModel payment) {
  final apiStatus = _normalizePaymentStatus(payment.status);
  final isSuccess =
      apiStatus == 'success' || apiStatus == 'paid' || apiStatus == 'completed';

  if (isSuccess) {
    return payment.status;
  }

  final isLocalCancelled = PaymentStatusOverrideNotifier.instance.isCancelled(
    paymentId: payment.paymentId,
    orderCode: payment.providerTransactionCode,
  );

  return isLocalCancelled ? 'Cancelled' : payment.status;
}
